...
import { Tag, ChevronDown, Close, Plus } from '../../icons';
import { useDropdown } from '../../../hooks/useDropdown';
import { ColorPicker } from '../../shared/ColorPicker';
import { ConfirmDialog } from '../../modals/ConfirmDialog';
import { Category } from '../../../types/category';
import { Habit } from '../../../types/habit';
import { COLOR_DISPLAY_MAP, TEXT_LENGTH_LIMITS } from '../../../constants';
...
/**
 * Выпадающий список для управления и выбора категорий
 * 
 * Позволяет:
...
export const CategoryPicker: React.FC<CategoryPickerProps> = ({
  ...
  const handleDeleteCategory = (categoryName: string) => {
    const usageCount = allHabits.filter(h => h.category === categoryName).length;
    ...
    <div className="relative flex items-center gap-2" ref={dropdown.ref} data-picker="category">
      ...
      <div className="flex-1 relative">
        ...
        >
          ...
          <span className="flex-1">{selectedCategory || 'Без категории'}</span>
        ...
        {dropdown.isOpen && (
          <div className="absolute left-0 top-full mt-1 z-50 bg-white border border-gray-200 rounded-lg shadow-lg w-full max-h-60 overflow-y-auto">
            ...
            {/* Список категорий */}
            {categories.map((cat) => {
              const usageCount = allHabits.filter(h => h.category === cat.name).length;
          ...
          message={
            deletingCategory.usageCount > 0
              ? `Категория "${deletingCategory.name}" используется в ${deletingCategory.usageCount} ${
                  deletingCategory.usageCount === 1 ? 'привычке' : 'привычках'
                }. Удалить категорию? Она будет удалена из всех привычек.`
