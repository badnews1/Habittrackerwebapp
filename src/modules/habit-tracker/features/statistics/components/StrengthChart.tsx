/**
 * График прогресса выполнения привычек с редактируемыми целями
 * 
...
import { DateConfig, GoalConfig } from '@/modules/habit-tracker/features/habits/types';
import { isHabitCompletedForDate, getMonthlyGoalFromFrequency } from '@/modules/habit-tracker/features/habits/utils';
import { calculateStrengthHistory } from '@/modules/habit-tracker/features/strength/utils';
...
export function StrengthChart({
  ...
  const [strengthHistory, setStrengthHistory] = useState<number[]>([]);
  const [topHabits, setTopHabits] = useState<{ name: string, percentage: number, completedCount: number }[]>([]);

  useEffect(() => {
    const history = calculateStrengthHistory(habits, monthDays, formatDate);
    setStrengthHistory(history);
  }, [habits, monthDays, formatDate]);
