...
import { DEFAULT_FREQUENCY_VALUES } from '../types/frequency';
import { validateCount, validatePeriod } from '../utils/frequencyValidation';
 ...
 * - Пропсы используются только для начального состояния
 * - Изменения идут через колбэки наверх
 * - Локальное состояние обновляется только через хендлеры
 * 
 * Оптимизация производительности:
 * - useRef для localValues - избегает пересоздания колбэков
 * - useRef для пропс-колбэков - стабильные ссылки
 * - Функциональное обновление setState
 * - Все useCallback с пустым массивом зависимостей
 * 
 * Синхронизация ref:
 * - localValuesRef обновляется ТОЛЬКО через useEffect
 * - НЕТ обновлений ref внутри setState (anti-pattern!)
 * - useEffect гарантирует синхронизацию после коммита
 * - Избегаем проблем с батчингом React
 * 
 * Валидация значений:
 * - Проверка на NaN через Number.isNaN()
 * - Проверка на отрицательные числа
 * - Проверка на ноль
 * - Автоматический clamping к максимальным значениям
 * - Специальная логика для n_times_in_m_days (count <= period)
 * 
 * Безопасность типов:
 * - Явные проверки типов через if/else вместо type casting
 * - Нет использования "as keyof typeof" для небезопасного доступа
 * - Early return для неподдерживаемых типов
 * 
 * Дата создания: 19 ноября 2024
 * Последнее обновление: 19 ноября 2024 (убраны обновления ref из setState)
 */
export const useFrequencyState = (props: FrequencyEditorProps) => {
   ...
   * 
   * ОПТИМИЗАЦИЯ: Использует Refs вместо зависимостей → НЕ пересоздаётся
   * ГАРАНТИЯ СИНХРОННОСТИ: Обновляет localValuesRef ПЕРЕД вызовом родительских колбэков
   * ТИПОБЕЗОПАСНОСТЬ: Exhaustive check гарантирует покрытие всех типов
   */
   ...
   * ОПТИМИЗАЦИЯ: Использует callbacksRef
   * ВАЛИДАЦИЯ: Проверяет на NaN, отрицательные числа, максимумы
   */
   ...
   * ОПТИМИЗАЦИЯ: Использует callbacksRef
   * БЕЗОПАСНОСТЬ: Явная проверка типов вместо type casting
   * АКТУАЛЬНОСТЬ: Берёт значение из ref, а не из параметра
   */
  ...
  // ОПТИМИЗАЦИЯ: Теперь зависит ТОЛЬКО от localValues, т.к. все колбэки стабильны
  return useMemo(() => ({
    localValues,
    handleTypeChange,
    handleCountChange,
