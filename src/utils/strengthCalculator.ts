...
import { EMA_PERIOD } from '../constants/strength';
import { strengthLogger } from './logger';
...
export const recalculateStrength = (habit: Habit, changedDate?: string): Habit => {
  ...
  if (lastUpdateStr === todayStr && habit.lastStrengthUpdate && !isChangedBeforeLastUpdate) {
    ...
    while (currentDate <= today) {
      ...
      // Логика заморозки: намереннй пропуск (✗) не меняет силу
      if (isSkipped) {
        // ЗАМОРОЗКА: сила не меняется
        strengthLogger.debug(`  ${dateStr}: ✗ FROZEN - strength stays at ${strength.toFixed(2)}%`);
      } else {
        // Получаем пропорциональное начение выполнения (0-100)
  ...
  if (allDates.length > 0) {
    ...
    if (earliestDate < startDate) {
      strengthLogger.debug('Found earlier date with completions:', earliestDateStr, '(before createdAt:', createdAt.toISOString().split('T')[0], ')');
  ...
  strengthLogger.group(`Full recalculation (${reason}): ${habit.name}`, () => {
    strengthLogger.debug('Starting from scratch at:', currentDate.toISOString().split('T')[0]);
