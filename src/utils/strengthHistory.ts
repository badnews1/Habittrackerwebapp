/**
 * Утилиты для расчёта истории силы привычки (EMA алгоритм)
 * 
 * Этот модуль содержит универсальные функции для EMA расчётов,
 * которые используются как в инкрементальном обновлении силы (strengthCalculator.ts),
 * так и в построении графика истории (HabitStatisticsModal.tsx).
 * 
 * Создано: 20 ноября 2025
 * Цель: Унификация EMA логики, устранение дублирования
 * 
 * @module utils/strengthHistory
 */
 ...
 * 
 * Используется как точка отсчёта для построения истории.
 * 
 * @param habit - Привычка
 * @returns Самая ранняя дата или дата создания привычки
 */
export function findEarliestDateWithData(habit: Habit): Date {
...
/**
 * Рассчитывает силу привычки на конкретную дату
 * 
 * Быстрый способ получить силу на определённую дату без построения полной истории.
 * 
 * @param habit - Привычка
 * @param targetDate - Целевая дата (строка в формате YYYY-MM-DD)
 * @param options - Опции расчёта
 * @returns Значение силы на указанную дату
 * 
 * @example
...
export function calculateStrengthAtDate(
  habit: Habit,
  targetDate: string,
  options: Omit<StrengthHistoryOptions, 'endDate'> = {}
): number {
  ...
  const history = calculateStrengthHistory(habit, {
    ...options,
    endDate: target,
    round: false // Не округляем для точности
  });
  
  // Возвращаем последнюю точку (это и есть сила на целевую дату)
  if (history.length === 0) {
    return options.initialStrength || 0;
  }
  
  return history[history.length - 1].strength;
}
