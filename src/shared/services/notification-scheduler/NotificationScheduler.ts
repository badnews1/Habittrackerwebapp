 ...
 * 
 * @module shared/services/notification-scheduler
...
class NotificationSchedulerClass {
  ...
  private async triggerNotifications(time: string): Promise<void> {
    ...
    console.log(`[NotificationScheduler] Показ ${reminders.length} уведомлений на ${time}`);
    
    // Проверяем нужно ли группировать
    if (this.shouldGroup(reminders)) {
      await this.showGroupedNotification(reminders);
    } else {
