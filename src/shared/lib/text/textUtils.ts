 ...
 * 
 * @param unit - Полное название единицы измерения (на русском или английском)
 * @returns Сокращённое название
 * 
 * @example
 * getShortUnit('километры') // "км"
 * getShortUnit('kilometers') // "km"
 * getShortUnit('литры') // "л"
 * getShortUnit('liters') // "l"
 */
...
export function formatNumber(num: number): string {
  if (num < 10000) {
    ...
    // Если округление дало >= 1000k, показываем ��ак M
    if (k >= 1000) {
      return '1M';
    }
    return `${k}k`;
