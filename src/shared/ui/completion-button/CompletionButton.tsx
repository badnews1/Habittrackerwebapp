...
export interface CompletionButtonProps {
  ...
  variant?: 'completed' | 'skipped' | 'empty' | 'partial';
  /** Обработчик клика */
...
export function CompletionButton({
  ...
  const variantColors = {
    completed: 'var(--accent-dark)',      // Галочка
    skipped: 'var(--bg-secondary)',          // Пауза
    empty: 'var(--bg-secondary)',            // Пустой
    partial: 'transparent',                  // Прозрачный для частичного выполнения
  };

  // Маппинг вариантов на цвет иконок (через CSS переменные)
  const iconColors = {
    completed: 'var(--bg-primary)',          // Галочка
    skipped: 'var(--text-secondary)',        // Пауза
    empty: 'transparent',                     // Нет иконки
  ...
  return (
    <button
      ...
      className={cn(
        // Базовые стили
        'w-5 h-5 rounded-full transition-all hover:scale-105 flex items-center justify-center flex-shrink-0',
        // Кастомные классы
        className
      )}
      style={{
