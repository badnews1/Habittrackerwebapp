...
i18next автоматически выбирает правильную форму слова в зависимости от числа и языка.

### Правила именования ключей:

**Английский (EN)** - 2 формы:
- `key_one` - для числа 1
- `key_other` - для всех остальных чисел (0, 2, 3, ...)

**Русский (RU)** - 3 формы:
- `key_one` - для чисел, оканчивающихся на 1 (кроме 11): 1, 21, 31...
- `key_few` - для чисел, оканчивающихся на 2-4 (кроме 12-14): 2, 3, 4, 22, 23...
- `key_many` - для всех остальных: 0, 5-20, 25-30...

### Примеры в переводах:

```typescript
// /shared/locales/en/common.ts
{
  plurals: {
    day_one: "day",
    day_other: "days",
...
// i18next автоматически выберет правильную форму
t('plurals.day', { count: 1 })  // "day" / "день"
t('plurals.day', { count: 2 })  // "days" / "дня"
t('plurals.day', { count: 5 })  // "days" / "дней"
t('plurals.day', { count: 21 }) // "days" / "день"

// С интерполяцией в тексте
// EN: { "items_one": "{{count}} item", "items_other": "{{count}} items" }
// RU: { "items_one": "{{count}} элемент", "items_few": "{{count}} элемента", "items_many": "{{count}} элементов" }
t('items', { count: 5 }) // "5 items" / "5 элементов"
```

### ⚠️ Важно:

- **НЕ используйте ручное склонение** (функции типа `pluralize(count, 'дело', 'дела', 'дел')`)
- **Всегда используйте i18n pluralization** для корректной работы на всех языках
- i18next **автоматически определяет** правильную форму на основе языка и числа
