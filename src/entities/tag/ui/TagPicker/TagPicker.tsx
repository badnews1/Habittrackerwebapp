...
export function TagPicker<T extends BaseTag = BaseTag>({
  ...
  const handleAddTag = () => {
    ...
    console.log('🏷️ TagPicker: handleAddTag вызван', { 
      normalized, 
      newTagColor,
      currentTags: tags?.map(t => t.name) || [],
      currentTagsCount: tags?.length || 0,
    });
    ...
    if (!normalized || alreadyExists) {
      console.log('⚠️ TagPicker: Тег не добавлен', { normalized, alreadyExists });
      return;
    }
    
    console.log('✅ TagPicker: Вызываем onAddTag', { normalized, newTagColor });
    onAddTag(normalized, newTagColor);
    
    // Автоматически добавляем новый тег в выбранные
    console.log('✅ TagPicker: Добавляем тег в выбранные', { 
      selectedTags, 
      normalized,
      newSelected: [...selectedTags, normalized],
    });
    ...
    console.log('✅ TagPicker: Состояние сброшено');
  };
