...
export const FrequencyTwoColumn: React.FC<FrequencyTwoColumnProps> = ({
  ...
  const renderParameters = () => {
    switch (frequency.type) {
        ...
        return (
          <div className="grid grid-cols-7 gap-2">
            {weekdaysArray.map((day, index) => {
              const isSelected = frequency.daysOfWeek.includes(index);
              return (
                <Toggle
                  key={index}
                  pressed={isSelected}
      ...
      case 'every_n_days':
        return (
          <div className="flex items-center gap-3">
            <span className="text-sm text-text-secondary">{t('frequencyConfig.every')}</span>
            <Input
              type="number"
              min={1}
              max={365}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[160px_1fr] gap-4 md:gap-6">
      ...
      <div className="min-h-[80px] flex items-start pt-7">
        {renderParameters()}
      </div>
    </div>
  );
